...
Clicking "New game" increments the seed (42 -> 43), so a second game is a *different*
...
```js
const el=[...document.querySelectorAll('section[aria-label="Enemy waters"] button')]
...
- A refused placement must keep the chosen ship selected (next legal click places the *same*
