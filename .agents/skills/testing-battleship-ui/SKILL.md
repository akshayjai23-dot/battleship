...
Production deploy: https://battleship-five-chi.vercel.app/ (accepts the same `?seed=42`).
It is a static Vite build, so behaviour matches localhost; still verify the geometry checks
against the deployed URL, since the CSS is what the responsive fix lives in.

## Move log numbering

`.log` uses `padding-left: 1.25rem` with default `list-style-position: outside`, so once the
log reaches **10+ entries** the two-digit marker is clipped and "10." renders as "0.".
Always play at least 10 moves before signing off on log numbering, and check the rendered
marker in a screenshot, not just `<ol reversed start="N">` in the DOM.
